...
public class KS01_ZeroOneKnapsack {
	...
	private static int knapSackRecursive(int[] weight, int[] value, int bagWeight, int n) {
		...
		if(weight[n-1] > bagWeight)
			...
			return Math.max(value[n-1] + knapSackRecursive(weight, value, bagWeight-weight[n-1], n-1), knapSackRecursive(weight, value, bagWeight, n-1));
	}
	...
	private static int knapSackRecMemoization(int[] weight, int[] value, int bagWeight, int n, int[][] dp) {
		...
		if(weight[n-1] > bagWeight)
			...
			return dp[n][bagWeight] = Math.max(value[n-1] + knapSackRecMemoization(weight, value, bagWeight-weight[n-1], n-1, dp), 
